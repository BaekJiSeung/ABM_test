...
def getTotalInfec(model):
    ...
    return model.totalHCWinf

# 컴파트먼트 카운트하려고 추가
def count_patients(model):
    return sum(1 for a in model.schedule.agents if getattr(a, "isPatient", False))

def count_colonized_patients(model):
    return sum(1 for a in model.schedule.agents
               if getattr(a, "isPatient", False) and getattr(a, "colonized", False))

def count_uncolonized_patients(model):
    return sum(1 for a in model.schedule.agents
               if getattr(a, "isPatient", False) and not getattr(a, "colonized", False))

def count_isolated_patients(model):
    return sum(1 for a in model.schedule.agents
               if getattr(a, "isPatient", False) and getattr(a, "isolated", False))

def count_positive_patients(model):
    return sum(1 for a in model.schedule.agents
               if getattr(a, "isPatient", False) and getattr(a, "positive", False))

def count_preinfected_patients(model):
    return sum(1 for a in model.schedule.agents
               if getattr(a, "isPatient", False) and getattr(a, "preinfection", False))

def count_hcws(model):
    return sum(1 for a in model.schedule.agents if getattr(a, "isHCW", False))

def count_colonized_hcws(model):
    return sum(1 for a in model.schedule.agents
               if getattr(a, "isHCW", False) and getattr(a, "colonized", False))

def count_goo(model):
    return sum(1 for a in model.schedule.agents if getattr(a, "isGoo", False))

def count_colonized_goo(model):
    return sum(1 for a in model.schedule.agents
               if getattr(a, "isGoo", False) and getattr(a, "colonized", False))

def count_beds(model):
    return sum(1 for a in model.schedule.agents if getattr(a, "isBed", False))

def count_filled_beds(model):
    return sum(1 for a in model.schedule.agents
               if getattr(a, "isBed", False) and getattr(a, "filled", False))

def count_filled_sick_beds(model):
    return sum(1 for a in model.schedule.agents
               if getattr(a, "isBed", False) and getattr(a, "filledSick", False))
...
class CPE_Model_month(Model):
    ...
    tau_offset_days):       # [ADD] 대청소 주기 (기본 180일))
        ...
        self.tau_offset_days =  int(tau_offset_days)
        ...
        # [ADD] 초기오염 지정/대청소 트리거용 도우미
        self.goo_pool = []          # Goo 에이전트 핸들 저장
        ...
        self.cumul_sick_patients_by_HCW = 0
        self.num_infecByHCW = 0 # HCW에 의한 감염환자 수 (*)
        self.totalHCWinf = [] # list for infected people 
        self.P_I = 0
        self.num_move2isol = 0 # 검사로 인해 Isolated bed로 옮겨지는 환자 수 (*)
        self.daily_discharges = 0 
        self.daily_admissions = 0
        # 컴파트먼트위해 히스토리 추가
        self.history = {
            "day": [],
            "patients": [],
            ...
            "cumulative_sick_patients_by_HCW": [],   # [ADD]
            "cumulative_patients": [],
            "move2isol": [],
            "daily_discharges": [], 
            "daily_admissions": []
        }
        self.monthly_hcw_infections = []
        ...
        lambda_param = 1/self.hospital_period  # 지수 분포의 람다(lambda) 파라미터
        ...
        for i in range(30):
            ...
            self.goo_pool.append(d) #[ADD] goo등록
        #[NEW] 초기 환경오염: 침대 1번부터 순서대로 init_env_beds개 Goo를 colonized=True
        if self.goo_pool:
            k = max(0, min(self.init_env, len(self.goo_pool)))
            for g in self.goo_pool[:k]:
                g.colonized = True
...
# 컾파ㅡ먼트위해 출력할것
    def record_daily_state(self):
        ...
        self.history["patients"].append(count_patients(self))
        self.history["patient_C"].append(count_colonized_patients(self))
        self.history["patient_S"].append(count_uncolonized_patients(self))
        self.history["patient_isolated"].append(count_isolated_patients(self))
        self.history["patient_positive"].append(count_positive_patients(self))
        self.history["patient_preinfection"].append(count_preinfected_patients(self))
        self.history["hcws"].append(count_hcws(self))
        self.history["hcw_C"].append(count_colonized_hcws(self))
        self.history["goo"].append(count_goo(self))
        self.history["goo_C"].append(count_colonized_goo(self))
        self.history["beds"].append(count_beds(self))
        self.history["filled_beds"].append(count_filled_beds(self))
        self.history["filled_sick_beds"].append(count_filled_sick_beds(self))
        self.history["empty_isolated_beds"].append(len(self.empty_beds))
        self.history["daily_hcw_infections"].append(self.num_infecByHCW)
        self.history["cumulative_sick_patients"].append(self.cumul_sick_patients)
        self.history["cumulative_sick_patients_by_HCW"].append(self.cumul_sick_patients_by_HCW)
        self.history["cumulative_patients"].append(self.cumul_patients)
        self.history["move2isol"].append(self.num_move2isol)
        self.history["daily_discharges"].append(self.daily_discharges)
        self.history["daily_admissions"].append(self.daily_admissions)   
    ...
    def step(self):
        ...
        # 일별 HCW infection 저장
            self.totalHCWinf.append(self.num_infecByHCW)
        ...
        #     #self.totalHCWinf.append(self.num_infecByHCW) ##revised_by_Wi
