# %% ================== 01_summary_mapping_isolation_B260.py ==================
# LONG raw ABM output -> isolation-specific summary CSV
# summary CSV -> isolation-specific beta_ABM -> beta_SM mapping
#
# Input:
#   ../result/interv_prob_transmission_LONG_B260_0.02-0.06_isoltimeALL.csv
#
# Output summary:
#   ../result/interv_prob_transmission_summary_B260_0.02-0.06_isoltime6.csv
#   ../result/interv_prob_transmission_summary_B260_0.02-0.06_isoltime14.csv
#   ../result/interv_prob_transmission_summary_B260_0.02-0.06_isoltime20.csv
#   ../result/interv_prob_transmission_summary_B260_0.02-0.06_isoltime28.csv
#   ../result/interv_prob_transmission_summary_B260_0.02-0.06_isoltimeALL.csv
#
# Output mapping:
#   sm_fit/theta_pairs_subset_cumGaussian_B260_isoltime6.csv
#   sm_fit/theta_pairs_subset_cumGaussian_B260_isoltime14.csv
#   sm_fit/theta_pairs_subset_cumGaussian_B260_isoltime20.csv
#   sm_fit/theta_pairs_subset_cumGaussian_B260_isoltime28.csv
#   sm_fit/theta_pairs_subset_cumGaussian_B260_isoltimeALL.csv
...
def isol_to_tag(isol_time):
    return str(int(isol_time))
...
print("=" * 80)
print("START make isolation summary - Period B")
...
df_long["isolation_time"] = df_long["isolation_time"].astype(float)
...
if check_counts["n"].min() != num_iter or check_counts["n"].max() != num_iter:
    print("[WARNING] Some isolation-beta settings do not have num_iter runs.")
else:
    print("[OK] Every isolation-beta setting has num_iter runs.")

all_summary_rows = []

for isol_time in isolation_values:

    isol_tag = isol_to_tag(isol_time)

    print("\n" + "=" * 80)
    print("Summarizing isolation_time =", isol_time)
    print("avg_isolation_delay =", isol_time / 2)
    print("=" * 80)

    df_iso = df_long.loc[
        np.isclose(df_long["isolation_time"], isol_time)
    ].copy()

    if df_iso.empty:
        print("[WARNING] No rows for isolation_time =", isol_time)
        continue
    ...
    for beta in beta_values:
        ...
        sub = df_iso.loc[
            np.isclose(df_iso["prob_transmission"], beta)
        ].copy()

        print(f"isolation_time={isol_time}, beta={beta:.3f}, n={len(sub)}")
        ...
        row_out = {
            ...
            "isolation_time": float(isol_time),
            "avg_isolation_delay": float(isol_time / 2),
            "data_type": data_type,
            "init_env": init_env,
            "tau0": tau0
        }
    ...
    out_csv = os.path.join(
        ...
        f"{scenario_tag}_{beta_tag1}-{beta_tag2}_isoltime{isol_tag}.csv"
    )
...
#%%
# ==================================================
# 4. Isolation surrogate model
# ==================================================

def simulate_theta(beta, init_env, tau0, isol_time,
                   monthly_PI=monthly_PI,
                   pi_dates=PI_dates):
    """
    Period B surrogate model for isolation intervention.
    ...
    isol_time  : isolation transition time
    """
...
def model_monthly_and_cum(theta, isol_time, months, init_env=init_env, tau0=tau0):
...
def negloglik_theta_cum_gaussian(theta, cum_obs, cum_std,
                                 months, init_env, tau0, isol_time):
...
def fit_theta_cum_gaussian_for_one(beta_abm, y_mean, y_std, isol_time, months):
...
def fit_one_row(row_dict, isol_time, months):
    ...
    print(
        f"done: isolation_time={isol_time}, beta_ABM={beta_abm:0.3f}, "
...
def load_abm_summary(isol_time):
    isol_tag = isol_to_tag(isol_time)
...
for isol_time in isolation_values:
    ...
    print("\n" + "=" * 80)
    print(f"START fitting for isolation_time = {isol_time}")
    print("=" * 80)

    df_abm = load_abm_summary(isol_time)
    ...
    out_csv = os.path.join(
        ...
        f"{scenario_tag}_isoltime{isol_tag}.csv"
    )
...
df_all = (
    df_all
    .sort_values(["isolation_time", "beta_abm"])
    .reset_index(drop=True)
)
...
print("\nBoundary check by isolation_time:")
print(
    df_all
    .assign(
    ...
    .groupby("isolation_time")[["at_lower", "at_upper"]]
    .sum()
)

print("\nDONE isolation-specific mapping fitting - Period B")
# %%
